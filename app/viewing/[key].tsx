...
import {
  ...
  setApplicantStatus, appendApplicantNote, MondayError,
} from '../../lib/monday';
import {
      ...
      <Card>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 4 }}>
          ...
          <Text style={styles.expectedHint}>
            {expectedRefs.length > 0
              ? 'From the Applicants column on the schedule board — tap a name to mark them as attended.'
        ...
        {expectedRefs.length === 0 && !loadingExpected && (
          <Text style={styles.expectedEmpty}>
            If names should be showing here, check that the Applicants column ID from the schedule board is
            filled in under Settings → Monday.com, then pull down on the Viewings list to refresh.
          </Text>
        )}
